...
class EegListenerFilterPowerSender:
    ...
    def startReadAndProcess(self):
        ...
        while not mustStop:
            ...
            self.buff[self.bufferSize - 1, :], timestamp = self.inlet.pull_sample()
    ...
    def processBuffer(self, band = "Alpha"):
        return [bandPower1.freqBandsPower(self.buff[:,channel], self.inputRate)[band] for channel in xrange(self.channels)]
        
        
if __name__ == "__main__":
    worker = EegListenerFilterPowerSender()
    worker.startReadAndProcess()
